...
function InputOne(e){
	...
	if (e.target.value.length < 101) {windowStore.dispatch({type : "CHANGENAME" , text : e.target.value })}

}

function ClickCLack(e){
	leftpanel_store.getState().data.map( (i) => {unsick({i})}  )
}

function unsick(props){
	if (props.i.values!=="none") {props.i.values.map( (i) => { unsick({i})}  )}	
